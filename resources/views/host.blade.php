...
<body class="min-h-screen flex flex-col justify-between overflow-x-hidden">
    ...
    <main class="container mx-auto px-6 py-8 flex-grow grid lg:grid-cols-4 gap-8 z-10">
        ...
        <div class="lg:col-span-1 flex flex-col gap-6">
            ...
            <!-- Connectivity Card -->
            <div class="theme-card p-6 rounded-2xl shadow-xl">
                <h3 class="theme-text-muted text-xs font-bold uppercase tracking-wider mb-4">Mode Jaringan</h3>
                <div id="network-badge" class="px-3.5 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 mb-3">
                    Mendeteksi...
                </div>
                <p id="network-desc" class="text-[11px] theme-text-muted leading-relaxed text-center">
                    Mendeteksi lingkungan jaringan...
                </p>
            </div>
    ...
    <script>
        ...
        const btnCopyLinkDesc = document.querySelector('#btn-copy-link + span');
        ...
        if (isLocalNetwork) {
            networkBadge.className = "px-3.5 py-2 rounded-xl text-xs font-bold bg-cyan-500/10 border border-cyan-500/20 text-cyan-500 flex items-center justify-center gap-1.5 mb-3";
            networkBadge.innerHTML = `<span class="h-2 w-2 rounded-full bg-cyan-500 animate-pulse"></span> Jaringan Lokal (Wi-Fi/LAN)`;
            networkDesc.textContent = "Sangat cepat & hemat kuota. Hanya perangkat dalam satu Wi-Fi/LAN yang dapat bergabung.";
            if (btnCopyLinkDesc) btnCopyLinkDesc.textContent = "Bagikan tautan ini ke penonton di jaringan lokal yang sama.";
        } else {
            networkBadge.className = "px-3.5 py-2 rounded-xl text-xs font-bold bg-blue-500/10 border border-blue-500/20 text-blue-400 flex items-center justify-center gap-1.5 mb-3";
            networkBadge.innerHTML = `<span class="h-2 w-2 rounded-full bg-blue-500 animate-pulse"></span> Online (Internet)`;
            networkDesc.textContent = "Dapat diakses dari mana saja. Memerlukan koneksi internet dan wajib HTTPS untuk membagikan layar.";
            if (btnCopyLinkDesc) btnCopyLinkDesc.textContent = "Bagikan tautan ini ke siapa saja di internet untuk bergabung.";
        }
    </script>
</body>
</html>